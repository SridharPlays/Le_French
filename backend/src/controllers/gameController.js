...
export const getUserProgress = async (req, res) => {
  try {
    ...
    const totalXp = await pool.query(
        'SELECT SUM(xp_gained) as total FROM user_progress WHERE user_id = $1',
        [req.user.id]
    );
...
export const addProgress = async (req, res) => {
  const { lesson_id, xp_gained } = req.body;
  const user_id = req.user.id;

  if (!lesson_id || !xp_gained) {
    console.log("Missing Data:", { lesson_id, xp_gained });
    return res.status(400).json({ msg: 'Missing lesson_id or xp_gained' });
  }

  try {
    const newProgress = await pool.query(
      'INSERT INTO user_progress (user_id, lesson_id, xp_gained) VALUES ($1, $2, $3) RETURNING *',
      [user_id, String(lesson_id), xp_gained]
    );
  ...
  try {
    ...
    const query = `
      ...
      WHERE
        u.batch_id = $1
      GROUP BY
        u.user_id, u.name, u.email, u.registration_number
      ORDER BY
        total_xp DESC;
    `;
